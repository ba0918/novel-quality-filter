...
### ストア: `scores`（開幕話の形式判定対応で追加）

| フィールド     | 型       | 説明                                                                          |
| -------------- | -------- | ----------------------------------------------------------------------------- |
| `openingType`  | `string` | 開幕話の形式判定結果（normal / character-intro / bulletin-board / too-short） |
| `sampledCount` | `number` | 採点に使用した話数（1 = 第1話のみ）                                           |

フィールド追加により IndexedDB
スキーマが変わるため、スキーマバージョンを更新して既存キャッシュを一度無効化する。キャッシュの有効期限は従来どおり無期限。

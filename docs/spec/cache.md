...
既存キャッシュを一度無効化する（現行スキーマバージョン: 7。v7 は `lineMetadata`
の各カテゴリ（`CategoryCount` / `NarrativeCount`）に短行14 / 短チャンク14
の集計フィールドを追加したことによる（型上は必須フィールドで宣言しているため、旧 v6
キャッシュは復元時に型と実体が乖離するので無効化する）。v6

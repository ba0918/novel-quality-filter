...
| フィールド     | 型             | 説明                                                                                                                                                                                                                                                                                                      |
| -------------- | -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `lineMetadata` | `LineMetadata` | 採点対象本文の行ベース診断メタデータ（6カテゴリの層別カウント）。基本は診断・表示用だが、`narrative.short14` と `narrative.lineCount` は「地の文短行14 の過多」ペナルティの入力にも用いる（scoring.md / line-metadata.md 参照）。本文抽出に失敗した話や地の文0行の話では未添付（undefined）／適用外となる |

フィールド追加やスコアリング仕様の変更で保存済みスコアが無効になる場合、スキーマバージョンを更新して
既存キャッシュを一度無効化する（現行スキーマバージョン: 9。v9 は表示スコア較正カーブ （scoring.md §

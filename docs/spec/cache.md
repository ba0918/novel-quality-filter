...
| フィールド     | 型             | 説明                                                                                                                                                                                                                                                                                                                                                                            |
| -------------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `lineMetadata` | `LineMetadata` | 採点対象本文の行ベース診断メタデータ（6カテゴリの層別カウント）。基本は診断・表示用だが、`narrative.charCount / lineCount` は M13（地の文の平均字/行）の weight 入力、`narrative.short14 / lineCount` は「地の文短行14 の過多」grade ペナルティの入力にも用いる（scoring.md / line-metadata.md 参照）。本文抽出に失敗した話や地の文0行の話では未添付（undefined）／適用外となる |

フィールド追加やスコアリング仕様の変更で保存済みスコアが無効になる場合、スキーマバージョンを更新して
既存キャッシュを一度無効化する（現行スキーマバージョン: 12。v12 は候補 D ── narrativeCharPerLine の
weight 化・Σweight rescale・短行14 penalty の grade 化・ 一文一段落過多の 0.85 緩和 ──
に伴う全作品のスコア変更による。v11 は penalty 合成の min-mult 化、v10 は 12 指標構造整地（4 指標の
weight 0 化と判別 4 指標への再配分）に よる。v9 は表示スコア較正カーブ （scoring.md §
表示較正カーブ）の導入に伴うスコア変更による。v8 で保存されたスコアは
較正カーブを通さない古い式で算出されているため、復元時に stale 判定して再算出させる。v8 は
「地の文短行14 の過多」ペナルティの導入に伴うスコア変更による。v7 は `lineMetadata`
